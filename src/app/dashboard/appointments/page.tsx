...
import { getAppointments, cancelAppointment, deleteAppointment, approveAppointment, getWaitlistEntries, deleteWaitlistEntry, generateTimeSlots, getSpecialists } from '@/lib/store';
import { Appointment, WaitlistEntry, Specialist } from '@/lib/types';
...
export default function AppointmentsPage() {
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const [aptData, waitData, specs] = await Promise.all([
                getAppointments(),
                getWaitlistEntries(),
                getSpecialists()
            ]);
    ...
    const handleDeleteWaitlist = async (id: string) => {
        if (confirm('Are you sure you want to delete this request from the waitlist?')) {
            try {
                ...
                {activeTab !== 'waitlist' && (
                    <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        {specialists.length > 1 && (
                            <>
                                <div style={{ width: '1px', height: '20px', background: 'var(--border-color)', margin: '0 var(--space-2)' }} />
                                <select
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', marginBottom: 'var(--space-2)' }}>
                                            ...
                                            <span className="badge badge-secondary" style={{ fontSize: '10px' }}>Waitlist</span>
                                        </div>
                                        <div style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', display: 'flex', flexDirection: 'column', gap: 'var(--space-1)' }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                                                ...
                                                Waiting for: {entry.preferredDates.join(', ')} at {entry.preferredTimes.join(', ')}
                                            </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
                                                                    ...
                                                                    {apt.verificationMethod && (
                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                                                                            <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                                                                                Verified via: {apt.verificationMethod === 'whatsapp' ? 'WhatsApp' : 'SMS'}
                                                                            </span>
                                                                        </div>
                                                                    )}
                                                                    {apt.notes && (
                                                                    ...
                                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                                                                        {approvingId === apt.id ? (
                                                                            <>
                                                                                ...
                                                                                >
                                                                                    <option value="none">This time only</option>
                        ...
                        {sortedDates.length === 0 && (
                            <div className="card" style={{ textAlign: 'center', padding: 'var(--space-12)' }}>
                                ...
                                <h3 style={{ color: 'var(--text-secondary)', marginBottom: 'var(--space-2)' }}>No appointments found</h3>
                                <p style={{ color: 'var(--text-muted)', margin: 0 }}>
                                    {filter !== 'all' ? 'Try changing your filters' : 'Share your link to receive bookings'}
                                </p>
                            </div>
                        )}
                    </div>
